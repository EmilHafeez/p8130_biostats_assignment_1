...
output: pdf_document
---
############NOTE THAT YOU COULD MAYBE CHANGE THE LINE LENGTH https://bookdown.org/yihui/rmarkdown-cookbook/text-width.html
...
ggplot(antibodies_df, aes(x = Antibody_IgM, fill = factor(Smell)) +
  geom_histogram((binwidth=.5, alpha=.5, position="identity", col = "black", fill = "red", breaks = seq(0.0400,1.05,.01)) +
    geom_vline(aes(xintercept=mean(Antibody_IgM, na.rm = TRUE)), color = "red", size = 1) +
  labs(title="Overlaid Histograms of Antibody Count by Smell Category") +                                             
  labs(x="Antibody Count(IgM)", y="Count") + 
  scale_fill_discrete(name = "Smell Category", labels = c("Normal", "Altered", "Other"))
```
